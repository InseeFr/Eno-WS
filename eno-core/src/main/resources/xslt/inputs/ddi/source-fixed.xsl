    ...
    <xsl:template name="enoddi:get-controlconstructreference-name">
        ...
        <xsl:choose>
            ...
            <xsl:when test="$type = 'Loop'">
                <xsl:variable name="temp" select="$root//d:Loop[r:ID=$id][1]//d:ControlConstructReference[r:TypeOfObject='QuestionConstruct'][1]/d:QuestionConstruct[1]//r:OutParameter[1]/r:ParameterName[1]/r:String[@xml:lang=$language][1]"/>
                <!-- Strange ! -->
                <xsl:value-of select="$temp[1]"/>
            </xsl:when>
        </xsl:choose>
